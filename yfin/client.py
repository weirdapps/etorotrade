...
class StockData:
    ...
    @property
    def _stock(self) -> "yf.Ticker": # Use string annotation for deferred import
        """Access the underlying yfinance Ticker object"""
        import yfinance as yf  # Import yfinance locally
...
class YFinanceClient:
    ...
    def __init__(self, retry_attempts: int = 3, timeout: int = 10, cache_ttl: int = 300):
        ...
        self.ny_tz = pytz.timezone('America/New_York')  # New York timezone for market hours
    ...
    def _adjust_earnings_date(self, date: pd.Timestamp) -> pd.Timestamp:
        """
        Adjust earnings date by adding one day since Yahoo reports the announcement date
        but we want the effective date (next trading day).
        """
        ...
        # Add one day to get the effective date
        return ny_date + timedelta(days=1)
    ...
    def get_past_earnings_dates(self, ticker: str) -> List[pd.Timestamp]:
        ...
        try:
            ...
            for date in earnings_dates.index:
                ...
                adjusted_date = self._adjust_earnings_date(date)
                dates.append(adjusted_date)

            # Filter past dates using current NY time
            current_time = pd.Timestamp.now(tz=self.ny_tz)
            past_dates = [date for date in dates if date < current_time]

            # Return sorted dates (most recent first)
            return sorted(past_dates, reverse=True)
