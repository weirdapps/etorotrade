...
class MarketDisplay:
    ...
    def generate_stock_report(self, ticker: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            stock_info = None  # Initialize stock_info to None
            ...
            # Get analyst ratings since last earnings date
            start_date = stock_info.last_earnings if stock_info else None
            ratings = self.analyst.get_ratings_summary(ticker, start_date=start_date)
            if not ratings:
                logger.info(f"No analyst ratings available for {ticker} since last earnings")
