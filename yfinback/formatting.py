...
class DisplayFormatter:
    ...
    def _get_color_code(self, 
        ...
        try:
            ...
            # No analyst ratings since earnings = low confidence
            if total_ratings == 0:
                return Color.YELLOW
    ...
    def format_value(self, value: Any, decimals: int = 1, percent: bool = False) -> str:
        ...
        if value is None or value in ["N/A", "--", ""]:
            return "--" if not percent else "0.0%"  # Show 0% for ratings when none since earnings
    ...
    def format_stock_row(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Handle ratings specifically - show 0 when none since earnings
            total_ratings = data.get("total_ratings")
            total_ratings = int(float(total_ratings or 0)) if total_ratings is not None else 0
            
            percent_buy = data.get("buy_percentage")
            percent_buy = float(percent_buy or 0) if percent_buy is not None else 0
            
            ex_ret = (upside * percent_buy / 100) if upside is not None and percent_buy is not None else 0
