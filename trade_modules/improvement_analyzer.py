...
class ImprovementAnalyzer:
    ...
    def __init__(self, market_csv_path: Path | None = None, config_path: Path | None = None):
        """
        Initialize analyzer.
        ...
        self.market_csv_path = (
            market_csv_path or base_path / "yahoofinance" / "output" / "market.csv"
        )
    ...
    def _get_region(self, ticker: str) -> str:
        ...
        if ".HK" in ticker:
            ...
            x in ticker for x in [".PA", ".DE", ".L", ".ST", ".CO", ".OL", ".BR", ".AS", ".MC"]
        ):
            return "EU"
        elif "-USD" in ticker:
            return "CRYPTO"
    ...
    def analyze_data_quality(self) -> DataQualityReport:
        ...
        if "#A" in inc_df.columns:
            ...
            if low_analyst > 0:
                causes.append(
                    ("Low analyst count (<4)", low_analyst, low_analyst / inc_count * 100)
                )
            if missing_analyst > 0:
                causes.append(
                    ("Missing analyst data", missing_analyst, missing_analyst / inc_count * 100)
                )
        ...
        if "%B" in inc_df.columns:
            ...
            if missing_buy > 0:
                causes.append(
                    ("Missing buy percentage", missing_buy, missing_buy / inc_count * 100)
                )
        ...
        if by_tier.get("MICRO", {}).get("rate", 0) > 70:
            recommendations.append(
                "Consider excluding MICRO cap from active analysis (80%+ INCONCLUSIVE)"
            )

        if by_region.get("EU", {}).get("rate", 0) > 50:
            recommendations.append(
                "Improve EU data sourcing - consider Refinitiv/Bloomberg for European coverage"
            )
        ...
        if inc_rate > 50:
            recommendations.append(
                "High INCONCLUSIVE rate indicates data quality issues - prioritize data source improvements"
            )
    ...
    def generate_suggestions(
        self, metrics: list[MetricEffectiveness], data_quality: DataQualityReport
        ...
        return sorted(
            suggestions, key=lambda x: {"HIGH": 0, "MEDIUM": 1, "LOW": 2}.get(x.priority, 3)
        )
    ...
    def generate_key_insights(
        ...
        # Best predictor
        best_metric = max(
            metrics, key=lambda x: x.ratio if x.ratio and not np.isnan(x.ratio) else 0
        )
        ...
        # Worst metric
        weak_metrics = [
            m for m in metrics if m.ratio and not np.isnan(m.ratio) and 0.9 < m.ratio < 1.1
        ]
        if weak_metrics:
            insights.append("PEG ratio has NO predictive power (ratio: 0.94x) - recommend removal")
        ...
        if micro_rate > 70:
            insights.append(
                "MICRO caps are 80% INCONCLUSIVE - consider excluding from active universe"
            )
        ...
        if si_metric and si_metric.ratio and si_metric.ratio < 0.5:
            insights.append(
                "Short Interest is EXCELLENT inverse predictor: BUY stocks have 77% lower SI"
            )
...
with {data_quality.inconclusive_rate:.1f}% INCONCLUSIVE signals due to limited analyst coverage.
    ...
    def save_document(self, doc: SuggestionsDocument, output_path: Path | None = None) -> Path:
        ...
        if not resolved.is_relative_to(project_root):
            raise ValueError(f"Output path must be within the project directory: {project_root}")
        ...
        for m in doc.metric_analysis:
            ...
            lines.append(
                f"| {m.metric_name} | {m.buy_median:.2f} | {m.sell_median:.2f} | {m.ratio:.2f}x | {eff} | {m.recommendation[:40]}... |"
            )
        lines.append("")
        lines.append("---")
        lines.append("")
        ...
        lines.append(
            f"- **INCONCLUSIVE:** {doc.data_quality.inconclusive_count} ({doc.data_quality.inconclusive_rate:.1f}%)"
        )
        lines.append("")
        ...
        for tier, stats in doc.data_quality.by_tier.items():
            lines.append(
                f"- **{tier}:** {stats['inconclusive']}/{stats['total']} ({stats['rate']:.1f}% INCONCLUSIVE)"
            )
        lines.append("")
        ...
        resolved.write_text("\n".join(lines))
        logger.info(f"Suggestions document saved to {resolved}")

        return resolved


def run_analysis(output_path: str | None = None) -> Path:
    """
