...
def _prepare_csv_dataframe(display_df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        logger.debug(
            f"Prepared CSV dataframe with {len(csv_df)} rows and {len(csv_df.columns)} columns"
        )
        return csv_df
        ...
        for col in colored_df.columns:
            if colored_df[col].dtype == "object":
                colored_df[col] = (
                    colored_df[col]
                    .astype(str)
                    .apply(lambda x: f"{color_code}{x}{COLOR_RESET}" if x != "" else x)
                )
...
def display_and_save_results(display_df: pd.DataFrame, title: str, output_file: str) -> None:
    ...
    try:
        ...
        table_output = tabulate(
            colored_df.values, headers=colored_df.columns, tablefmt="grid", colalign=alignments
        )
        print(table_output)
...
def format_display_dataframe(display_df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        # Format basic percentage columns with 1 decimal
        basic_percentage_columns = ["DIV%", "SI"]
        for col in basic_percentage_columns:
            if col in formatted_df.columns:
                formatted_df[col] = formatted_df[col].apply(_format_basic_percentage_value)

        # Format UPSIDE and EXRET with extreme value handling
        extreme_percentage_columns = ["UPSIDE", "EXRET"]
        for col in extreme_percentage_columns:
            if col in formatted_df.columns:
                formatted_df[col] = formatted_df[col].apply(_format_percentage_value)

        # Format %BUY column (no % suffix since it's in the header)
        if "%BUY" in formatted_df.columns:
            formatted_df["%BUY"] = formatted_df["%BUY"].apply(_format_buy_percentage_value)

        # Format specialized numeric columns
        if "BETA" in formatted_df.columns:
            formatted_df["BETA"] = formatted_df["BETA"].apply(_format_beta_value)

        if "PET" in formatted_df.columns:
            formatted_df["PET"] = formatted_df["PET"].apply(_format_pe_value)

        if "PEF" in formatted_df.columns:
            formatted_df["PEF"] = formatted_df["PEF"].apply(_format_pe_value)

        if "PEG" in formatted_df.columns:
            formatted_df["PEG"] = formatted_df["PEG"].apply(_format_peg_value)

        if "EG" in formatted_df.columns:
            formatted_df["EG"] = formatted_df["EG"].apply(_format_eg_pp_value)
        ...
        for col in earnings_columns:
            if col in formatted_df.columns:
                formatted_df[col] = formatted_df[col].apply(_format_date_value)
...
def _format_price_value(value: Any) -> str:
    """Format price values for display with conditional decimal places."""
    try:
        if pd.isna(value) or value is None or value == "" or value == "--" or value == 0:
            return "--"
...
def _format_percentage_value(value: Any) -> str:
    """Format percentage values for display with extreme value handling."""
    try:
        if pd.isna(value) or value is None or value == "" or value == "--" or value == 0:
            return "--"
        numeric_value = float(value)
...
def _format_date_value(value: Any) -> str:
    """Format date values for display without dashes (YYYYMMDD format)."""
    try:
...
def _format_size_value(value: Any) -> str:
    """Format position size values for display with 1 decimal."""
    try:
        if pd.isna(value) or value is None or value == "" or value == "--" or value == 0:
            return "--"
...
def _format_beta_value(value: Any) -> str:
    """Format beta values with extreme value handling and 1 decimal precision."""
    try:
        if pd.isna(value) or value is None or value == "" or value == "--" or value == 0:
            return "--"
        numeric_value = float(value)
        if numeric_value == 0:
            return "--"
        # Show H for >9, L for <-9, otherwise 1 decimal
        if numeric_value > 9:
            return "H"
...
def _format_pe_value(value: Any) -> str:
    """Format PE values (PET/PEF) with extreme value handling."""
    try:
        if pd.isna(value) or value is None or value == "" or value == "--" or value == 0:
            return "--"
        numeric_value = float(value)
        if numeric_value == 0:
            return "--"
        # Show H for >99, otherwise 1 decimal
        if numeric_value > 99:
            return "H"
        else:
            return f"{numeric_value:.1f}"
...
def _format_peg_value(value: Any) -> str:
    """Format PEG values with extreme value handling."""
    try:
        if pd.isna(value) or value is None or value == "" or value == "--" or value == 0:
            return "--"
        numeric_value = float(value)
        if numeric_value == 0:
            return "--"
        # Show H for >9, otherwise 1 decimal
        if numeric_value > 9:
            return "H"
        else:
            return f"{numeric_value:.1f}"
...
def _format_eg_pp_value(value: Any) -> str:
    """Format EG/PP values with extreme value handling."""
    try:
        if pd.isna(value) or value is None or value == "" or value == "--" or value == 0:
            return "--"
        numeric_value = float(value)
...
def _format_basic_percentage_value(value: Any) -> str:
    """Format basic percentage values with 1 decimal."""
    try:
        if pd.isna(value) or value is None or value == "" or value == "--" or value == 0:
            return "--"
        numeric_value = float(value)
...
def _format_buy_percentage_value(value: Any) -> str:
    """Format buy percentage values with % suffix, no decimals."""
    try:
        if pd.isna(value) or value is None or value == "" or value == "--" or value == 0:
            return "--"
        numeric_value = float(value)
...
class OutputManager:
    ...
    def generate_summary_report(self, results: Dict[str, Any]) -> str:
        ...
        try:
            ...
            total_analyzed = sum(len(df) for df in results.values() if isinstance(df, pd.DataFrame))
