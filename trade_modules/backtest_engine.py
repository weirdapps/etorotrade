...
class BacktestEngine:
    ...
    def fetch_price_history(
        ...
        try:
            ...
            logger.info("PriceService unavailable (%s), using direct yfinance", e)
            return self._fetch_price_history_legacy(tickers, start_date, end_date, batch_size)
    ...
    def _compute_group_stats(gdf: pd.DataFrame, signal_type: str) -> dict[str, Any]:
        ...
        return {
            ...
            "alpha_hit_rate": round(alpha_hr, 1)
            if alpha_hr is not None and not np.isnan(alpha_hr)
            else None,
            "alpha_hit_rate_ci_lo": round(alpha_ci_lo, 1)
            if alpha_ci_lo is not None and not np.isnan(alpha_ci_lo)
            else None,
            "alpha_hit_rate_ci_hi": round(alpha_ci_hi, 1)
            if alpha_ci_hi is not None and not np.isnan(alpha_ci_hi)
            else None,
    ...
    def print_summary(self, stats: dict[str, Any]) -> None:
        ...
        for h_key in sorted(stats.keys()):
            ...
            for sig in ["B", "S", "H"]:
                ...
                ahr = (
                    f"{s['alpha_hit_rate']:.1f}%" if s.get("alpha_hit_rate") is not None else "N/A"
                )
                print(
                    f"{name:<8} {s['count']:>7} {s['hit_rate']:>9.1f}% {ahr:>10} {mr:>10} {mdr:>10} {aa:>11} {flag}"
                )
            ...
            if h_stats.get("by_tier"):
                ...
                for tier in ["mega", "large", "mid", "small", "micro"]:
                    ...
                    aa = f"{s['avg_alpha']:.2f}%" if s.get("avg_alpha") is not None else "N/A"
                    ahr = (
                        f"{s['alpha_hit_rate']:.1f}%"
                        if s.get("alpha_hit_rate") is not None
                        else "N/A"
                    )
                    print(
                        f"{tier:<8} {s['count']:>7} {s['hit_rate']:>9.1f}% {ahr:>10} {mr:>10} {aa:>11} {flag}"
                    )

            print("\n  * = low sample size (n < 30)")

    def save_results(self, results_df: pd.DataFrame, stats: dict[str, Any]) -> None:
...
class ThresholdAnalyzer:
    ...
    def analyze_metric_predictiveness(self, merged: pd.DataFrame) -> list[dict[str, Any]]:
        """
    ...
    def analyze_sell_triggers(self, merged: pd.DataFrame) -> list[dict[str, Any]]:
        """
    ...
    def suggest_thresholds(self, merged: pd.DataFrame) -> list[dict[str, Any]]:
        """
        ...
        for region, tier in tier_region_combos:
            ...
            tier_data = merged[(merged["tier"] == tier) & (merged["region"] == region)]
            ...
            for config_key_name, metric_col in buy_metrics.items():
                ...
                suggestion = self._find_optimal_threshold(
                    buy_data, metric_col, current_val, direction="min", signal="B"
                )
                if suggestion:
            ...
            for config_key_name, metric_col in sell_metrics.items():
                ...
                suggestion = self._find_optimal_threshold(
                    sell_data, metric_col, current_val, direction="max", signal="S"
                )
                if suggestion:
                    suggestion["tier_region"] = config_key
                    suggestion["config_key"] = config_key_name
    ...
    def _print_report(
        ...
        regime_stats: list[dict[str, Any]] | None = None,
    ) -> None:
        ...
        if trigger_stats:
            ...
            for t in trigger_stats[:15]:  # Top 15
                label = (
                    "EFFECTIVE"
                    if t["hit_rate"] >= 65
                    else ("MODERATE" if t["hit_rate"] >= 55 else "WEAK")
                )
                print(
        ...
        if regime_stats:
            ...
            print(
                f"  {'Regime':<12} {'Signal':<6} {'Count':>7} {'Hit Rate':>10} {'α Hit Rate':>11} {'Mean Ret':>10} {'Avg Alpha':>11}"
            )
            print("  " + "-" * 69)
            for rs in sorted(regime_stats, key=lambda x: (x["regime"], x["signal"])):
                ...
                ahr = (
                    f"{rs['alpha_hit_rate']:.1f}%"
                    if rs.get("alpha_hit_rate") is not None
                    else "N/A"
                )
                print(
