            ...
            if portfolio_tickers:
                ...
                if "BS" in portfolio_df.columns:
                    ...
                    if not portfolio_sells.empty:
                        ...
                        sell_opportunities = pd.concat(
                            [sell_opportunities, portfolio_sells], ignore_index=False
                        )
                        sell_opportunities = sell_opportunities[
                            ~sell_opportunities.index.duplicated(keep="first")
                        ]
                        self.logger.info(
                            f"Added {len(portfolio_sells)} portfolio SELL stocks to sell opportunities"
                        )
                ...
                if "BS" in portfolio_df.columns:
                    ...
                    if not portfolio_sells.empty and ticker_col:
                        portfolio_sells = portfolio_sells.set_index(ticker_col)
                        sell_opportunities = pd.concat(
                            [filtered_market_sells, portfolio_sells], ignore_index=False
                        )
                        sell_opportunities = sell_opportunities[
                            ~sell_opportunities.index.duplicated(keep="first")
                        ]
                ...
                if "BS" in portfolio_df.columns:
                    ...
                    if not portfolio_holds.empty and ticker_col:
                        ...
                        hold_opportunities = pd.concat(
                            [filtered_market_holds, portfolio_holds], ignore_index=False
                        )
                        hold_opportunities = hold_opportunities[
                            ~hold_opportunities.index.duplicated(keep="first")
                        ]
                        self.logger.info(
                            f"Added {len(portfolio_holds)} portfolio HOLD stocks to hold opportunities"
                        )
                    else:
