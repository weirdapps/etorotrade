...
class DataFreshnessTracker:
    ...
    def __init__(self, signal_log_path: Path | None = None):
        self.signal_log_path = signal_log_path or SIGNAL_LOG_PATH

    def check_freshness(self, tickers: list[str] | None = None) -> dict[str, dict[str, Any]]:
        """
    ...
    def _analyze_ticker_freshness(
        ...
        if len(records) < 2:
            ...
            if records:
                return {
                    "days_since_change": (
                        today - datetime.strptime(records[0]["date"], "%Y-%m-%d").date()
                    ).days,
                    "staleness": "stale",
