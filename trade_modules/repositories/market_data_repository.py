...
class MarketDataRepository(IMarketDataRepository):
    ...
    def __init__(self, market_data_directory: str | Path):
        """
    ...
    def backup_market_data(self, backup_suffix: str | None = None) -> bool:
        """
    ...
    def consolidate_market_data(self, target_data_type: str = "consolidated") -> bool:
        ...
        try:
            available_types = [
                dt for dt in self.get_available_data_types() if dt != target_data_type
            ]
            ...
            if success:
                self.logger.info(
                    f"Consolidated {len(available_types)} data types into {target_data_type}"
                )
    ...
    def _is_valid_data_type(self, data_type: str) -> bool:
        ...
        return (
            data_type in self.STANDARD_DATA_TYPES
            or self.csv_repo.exists(data_type)
            or len(data_type) > 0
    ...
    def _validate_market_data(self, data: pd.DataFrame, data_type: str) -> bool:
        ...
        try:
            ...
            has_ticker_info = (
                ...
                or any(col in data.columns for col in ["ticker", "Ticker", "symbol", "Symbol"])
                or len(data.index) > 0  # Basic structure check
            )

            if not has_ticker_info:
                self.logger.warning(
                    f"Market data for {data_type} may be missing ticker information"
                )
