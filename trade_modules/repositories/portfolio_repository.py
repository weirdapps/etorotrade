...
class PortfolioRepository(IPortfolioRepository):
    ...
    def __init__(self, portfolio_file_path: str | Path):
        """
    ...
    def get_portfolio(self, **kwargs) -> pd.DataFrame:
        ...
        except (FileNotFoundError, pd.errors.EmptyDataError, OSError, KeyError, ValueError) as e:
    ...
    def get_holdings_for_ticker(self, ticker: str) -> dict[str, Any] | None:
        """
        ...
        try:
            ...
            if ticker_column is not None:
                ...
                if not holdings.empty:
                    ...
                    self.logger.debug(
                        f"Found holding for {ticker} in column {ticker_column}: {holding}"
                    )
                    return holding
            ...
            for idx in portfolio_df.index:
                if str(idx).upper() == ticker_upper:
                    ...
                    self.logger.debug(
                        f"Found holding for {ticker} via case-insensitive index search: {holding}"
                    )
                    return holding
    ...
    def add_holding(self, ticker: str, quantity: float, **kwargs) -> bool:
        ...
        try:
            ...
            new_holding = {"Ticker": ticker.upper(), "Quantity": quantity, **kwargs}
    ...
    def get_portfolio_summary(self) -> dict[str, Any]:
        ...
        try:
            portfolio_df = self.get_portfolio()
            if portfolio_df.empty:
                return {"total_holdings": 0, "total_value": 0.0, "total_quantity": 0.0}

            summary = {
                "total_holdings": len(portfolio_df),
                "holdings": portfolio_df.index.tolist()
                if hasattr(portfolio_df.index, "tolist")
                else [],
            }
    ...
    def backup_portfolio(self, backup_suffix: str | None = None) -> bool:
        """
    ...
    def _validate_portfolio_data(self, data: pd.DataFrame) -> bool:
        ...
        has_ticker_column = any(
            col in data.columns for col in ["Ticker", "ticker", "Symbol", "symbol"]
        )
        has_ticker_index = (
            data.index.name in ["Ticker", "ticker", "Symbol", "symbol"]
            if data.index.name
            else False
        )
        has_meaningful_index = len(data.index) > 0 and not data.index.equals(
