...
def load_signals(file_path: str) -> pd.DataFrame:
    ...
    for col in pct_cols:
        if col in df.columns:
            df[col] = (
                df[col].astype(str).str.rstrip("%").replace("--", np.nan).replace("nan", np.nan)
            )
...
def load_census(file_path: str) -> pd.DataFrame:
    ...
    for investor in investors:
        ...
        for holding in investor.get("portfolio", {}).get("positions", []):
            ...
            ticker = (
                holding.get("instrumentId") or holding.get("ticker") or holding.get("symbol", "")
            )
            if ticker:
                holdings.append(
                    {
    ...
    # Aggregate by ticker
    census_agg = df.groupby("ticker").agg({"weight": "mean", "investor": "count"}).reset_index()
...
def main():
    ...
    enriched = []
    for _idx, row in top_15.head(10).iterrows():
        ticker = row["TKR"]
        ...
        if row["sector"] == "Defense":
            macro_fit = (
                "Defense sector benefits from Iran escalation; increased military spending likely"
            )
    ...
    for _idx, row in top_15.iloc[10:].iterrows():
        matched = []
        if row.get("crit_a"):
            matched.append("A")
        if row.get("crit_b"):
    ...
    for opp in enriched:
        ...
        print(
            f"    Upside: {opp['upside_pct']:.1f}% | Buy%: {opp['buy_pct']:.0f}% | Beta: {opp['beta']:.1f}"
        )
