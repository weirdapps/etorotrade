...
- All Python dependencies are pinned with SHA256 hashes in `requirements-lock.txt` / `requirements-dev-lock.txt` / `requirements-smoketest-lock.txt` (committed). These are generated by `scripts/dev/relock.sh` from `pyproject.toml` + `poetry.lock`. CI installs via `pip install --only-binary :all: --require-hashes -r <lockfile>` for verified, sdist-free installs.
