...
def _format_alerts_html(alerts: List[Dict[str, Any]]) -> str:
    ...
    for alert in alerts[:20]:  # Show only the most recent 20 alerts
        timestamp = datetime.datetime.fromtimestamp(alert.get("timestamp", 0)).strftime("%Y-%m-%d %H:%M:%S")
...
def _format_circuit_breakers_html(circuit_breakers: Dict[str, Any]) -> str:
    ...
    for name, breaker in circuit_breakers.items():
        ...
        if last_failure:
            last_failure = datetime.datetime.fromtimestamp(last_failure).strftime("%Y-%m-%d %H:%M:%S")
        else:
            last_failure = "Never"
        
        last_success = breaker.get("last_success_time")
        if last_success:
            last_success = datetime.datetime.fromtimestamp(last_success).strftime("%Y-%m-%d %H:%M:%S")
        else:
            last_success = "Never"
