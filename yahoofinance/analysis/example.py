...
async def run_example():
    ...
    try:
        ...
        logger.info(f"AAPL analysis: {result.category} with expected return {result.expected_return}%")
        
        # Example 2: Async analysis
        result = await analyze_stock_async("MSFT")
        logger.info(f"MSFT analysis: {result.category} with expected return {result.expected_return}%")
        ...
        for ticker, result in batch_results.items():
            logger.info(f"{ticker} batch analysis: {result.category} with expected return {result.expected_return}%")
        ...
        try:
            ...
            logger.info(f"Portfolio analysis: {portfolio_summary.total_gain_loss_pct:.2f}% gain/loss")
