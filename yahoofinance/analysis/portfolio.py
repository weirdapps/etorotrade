...
class PortfolioHolding:
    ...
    def update_metrics(self):
        ...
        if self.current_price is not None:
            ...
            self.gain_loss_pct = (self.current_price / self.cost_basis - 1) * 100 if self.cost_basis > 0 else 0
...
class PortfolioAnalyzer:
    ...
    def __init__(
        ...
        self.stock_analyzer = stock_analyzer if stock_analyzer is not None else StockAnalyzer(provider=self.provider)
    ...
    def analyze_portfolio(self) -> PortfolioSummary:
        ...
        """
        if self.is_async:
            raise TypeError("Cannot use sync method with async provider. Use analyze_portfolio_async instead.")
    ...
    async def analyze_portfolio_async(self) -> PortfolioSummary:
        ...
        if not self.is_async:
            raise TypeError("Cannot use async method with sync provider. Use analyze_portfolio instead.")
    ...
    def _calculate_summary(self) -> None:
        ...
        for holding in self.holdings.values():
            ...
            if holding.analysis:
                ...
                sector = holding.analysis.ticker_info.get('sector', 'Unknown') if hasattr(holding.analysis, 'ticker_info') else 'Unknown'
                if sector != 'Unknown':
        ...
        performers = [(h.ticker, h.gain_loss_pct) for h in self.holdings.values() if h.gain_loss_pct is not None]
    ...
    def get_portfolio_dataframe(self) -> pd.DataFrame:
        ...
        for holding in self.holdings.values():
            data.append({
                ...
                'Expected Return': holding.analysis.expected_return if holding.analysis else None,
            })
    ...
    def get_allocation_dataframe(self) -> pd.DataFrame:
        ...
        """
        sectors = [(sector, allocation) for sector, allocation in self.summary.holdings_by_sector.items()]
