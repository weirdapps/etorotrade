...
class StockAnalyzer:
    ...
    def __init__(
        ...
        self.is_async = hasattr(self.provider, 'batch_get_ticker_info') and callable(self.provider.batch_get_ticker_info) and hasattr(self.provider.batch_get_ticker_info, '__await__')
    ...
    def analyze(self, ticker: str) -> AnalysisResults:
        ...
        """
        if self.is_async:
            raise TypeError("Cannot use sync method with async provider. Use analyze_async instead.")
    ...
    def analyze_batch(self, tickers: List[str]) -> Dict[str, AnalysisResults]:
        ...
        """
        if self.is_async:
            raise TypeError("Cannot use sync method with async provider. Use analyze_batch_async instead.")
    ...
    async def analyze_batch_async(self, tickers: List[str]) -> Dict[str, AnalysisResults]:
        ...
        """
        if not self.is_async:
            raise TypeError("Cannot use async method with sync provider. Use analyze_batch instead.")
        ...
        try:
            ...
            results = {}
            for ticker, result in [(item[0], item[1]) for item in results_list if not isinstance(item, Exception)]:
    ...
    def _process_analysis(
        ...
        analysis = AnalysisResults(
            ...
            prev_earnings_date=earnings_dates[1] if earnings_dates and len(earnings_dates) > 1 else None
        )
        ...
        if analyst_ratings:
            ...
            analysis.buy_count = analyst_ratings.get('strong_buy', 0) + analyst_ratings.get('buy', 0)
            analysis.hold_count = analyst_ratings.get('hold', 0)
            analysis.sell_count = analyst_ratings.get('sell', 0) + analyst_ratings.get('strong_sell', 0)
    ...
    def _apply_analysis_criteria(self, analysis: AnalysisResults) -> AnalysisResults:
        ...
        if analysis.pe_ratio is not None and analysis.forward_pe is not None and analysis.pe_ratio > 0 and analysis.forward_pe > 0 and analysis.pe_ratio > analysis.forward_pe:
            sell_signals.append(f"Deteriorating earnings outlook (PE {analysis.pe_ratio:.1f} > Forward PE {analysis.forward_pe:.1f})")
        ...
        if analysis.peg_ratio is not None and analysis.peg_ratio > self.max_peg_ratio:
            sell_signals.append(f"Overvalued relative to growth (PEG {analysis.peg_ratio:.1f} > {self.max_peg_ratio:.1f})")
        
        if analysis.short_percent is not None and analysis.short_percent > self.max_short_percent:
            sell_signals.append(f"High short interest ({analysis.short_percent:.1f}% > {self.max_short_percent:.1f}%)")
        
        if analysis.beta is not None and analysis.beta > 3.0:
            sell_signals.append(f"Excessive volatility (Beta {analysis.beta:.1f} > 3.0)")
        
        if analysis.expected_return is not None and analysis.expected_return < self.min_expected_return:
            sell_signals.append(f"Insufficient expected return ({analysis.expected_return:.1f}% < {self.min_expected_return:.1f}%)")
        ...
        if analysis.upside is None or analysis.upside < self.min_upside_buy:
            ...
            buy_signals.append(f"Strong upside potential ({analysis.upside:.1f}% > {self.min_upside_buy:.1f}%)")
        
        # Analyst consensus
        if analysis.buy_rating is None or analysis.buy_rating < self.min_buy_rating:
            buy_criteria_met = False
        else:
            buy_signals.append(f"Strong analyst consensus ({analysis.buy_rating:.1f}% > {self.min_buy_rating:.1f}%)")
        ...
        if analysis.pe_ratio is not None and analysis.forward_pe is not None:
            if analysis.pe_ratio <= 0:
                ...
                buy_signals.append(f"Improving earnings outlook (PE {analysis.pe_ratio:.1f} to Forward PE {analysis.forward_pe:.1f})")
        
        # Reasonable forward P/E
        if analysis.forward_pe is not None and (analysis.forward_pe <= 0.5 or analysis.forward_pe > 45.0):
            buy_criteria_met = False
        ...
        if analysis.peg_ratio is not None and analysis.peg_ratio > self.max_peg_ratio:
            ...
            buy_signals.append(f"Reasonable PEG ratio ({analysis.peg_ratio:.1f} < {self.max_peg_ratio:.1f})")
        
        # Acceptable short interest (if available)
        if analysis.short_percent is not None and analysis.short_percent > self.max_short_percent:
            buy_criteria_met = False
        elif analysis.short_percent is not None:
            buy_signals.append(f"Acceptable short interest ({analysis.short_percent:.1f}% < {self.max_short_percent:.1f}%)")
        
        # Expected return
        if analysis.expected_return is None or analysis.expected_return < self.min_expected_return:
            buy_criteria_met = False
        else:
            buy_signals.append(f"Strong expected return ({analysis.expected_return:.1f}% > {self.min_expected_return:.1f}%)")
        ...
        if buy_criteria_met:
            ...
            if analysis.upside is None or analysis.upside < self.min_upside_buy:
                analysis.signals.append(f"Insufficient upside ({analysis.upside:.1f if analysis.upside is not None else 'N/A'}% < {self.min_upside_buy:.1f}%)")
            
            if analysis.buy_rating is None or analysis.buy_rating < self.min_buy_rating:
                analysis.signals.append(f"Insufficient analyst consensus ({analysis.buy_rating:.1f if analysis.buy_rating is not None else 'N/A'}% < {self.min_buy_rating:.1f}%)")
