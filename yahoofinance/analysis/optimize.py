...
class PortfolioOptimizer:
    ...
    def get_historical_data(self) -> Tuple[pd.DataFrame, Set[str]]:
        ...
        for i in range(0, len(self.tickers), batch_size):
            ...
            logger.info(f"Processing batch {batch_num}/{total_batches} of {len(batch_tickers)} tickers")
        ...
        logger.info(f"Found historical data for {len(valid_tickers)} out of {len(self.tickers)} tickers")
    ...
    def calculate_returns(self, prices_df: pd.DataFrame) -> Dict[int, pd.DataFrame]:
        ...
        valid_tickers = [ticker for ticker, count in data_quality.items() if count >= min_data_points]
        ...
        if len(valid_tickers) < 5:
            ...
            logger.warning(f"Not enough tickers with sufficient data: only {len(valid_tickers)} tickers")
        ...
        for period in self.periods:
            ...
            try:
                ...
                if len(period_prices) < 20:  # Require at least 20 data points
                    logger.warning(f"Not enough data for {period}-year period (only {len(period_prices)} data points)")
                ...
                non_nan_ratio = daily_returns.notna().sum().sum() / (daily_returns.shape[0] * daily_returns.shape[1])
                if non_nan_ratio < 0.5:  # Require at least 50% non-NaN values
                    logger.warning(f"Too many NaN values in returns for {period}-year period (non-NaN ratio: {non_nan_ratio:.2f})")
    ...
    def optimize_portfolio(self, returns_df: pd.DataFrame) -> Dict:
        ...
        volatility = np.sqrt(np.dot(weights.T, np.dot(cov_matrix, weights))) * np.sqrt(252)  # Annualized
    ...
    def apply_constraints(
        ...
        if not optimization_results['weights'] is None:
            ...
            constrained_weights = [a / total_constrained if total_constrained > 0 else 0 
                                 for a in constrained_amounts]
            ...
            if returns_df is not None:
                ...
                volatility = np.sqrt(np.dot(np.array(constrained_weights).T, 
                                           np.dot(cov_matrix, constrained_weights))) * np.sqrt(252)
                sharpe_ratio = (expected_return - self.risk_free_rate) / volatility
            else:
            ...
            print(f"  All {len(tickers)} tickers included with at least ${self.min_amount:.2f} allocation")
    ...
    def get_current_prices(self) -> Dict[str, float]:
        ...
        print(f"Fetching current prices for {len(self.valid_tickers)} tickers in {total_batches} batches...")
        ...
        # Complete the progress line
        print(f"\rCurrent prices fetched for {len(current_prices)} of {len(self.valid_tickers)} tickers       ")
    ...
    def load_historical_data_from_cache(self) -> Tuple[pd.DataFrame, List[str]]:
        ...
        try:
            ...
            logger.info(f"Cache contains {len(valid_tickers)} tickers with {len(historical_data)} data points")
    ...
    def run_optimization(self) -> Dict[int, Dict]:
        ...
        start_time = time.time()
        self.load_portfolio()
        print(f"Portfolio loaded with {len(self.tickers)} tickers in {time.time() - start_time:.1f} seconds")
        ...
        print(f"Returns calculated for {len(returns_by_period)} time periods in {time.time() - start_time:.1f} seconds")
        ...
        if self.use_cache:
            ...
            current_prices = self.get_current_prices()
        print(f"Retrieved prices for {len(current_prices)} tickers in {time.time() - start_time:.1f} seconds")
    ...
    def display_results(self, results_by_period: Dict[int, Dict]) -> None:
        ...
        if not results_by_period:
            ...
            print("This may be because no valid historical data was found for your portfolio tickers.")
        ...
        for period, results in sorted(results_by_period.items()):
            ...
            if 'tickers' in results:
                print(f"Portfolio includes all {len(results['tickers'])} tickers with at least ${self.min_amount:.2f} allocation")
            ...
            amounts = results.get('amounts', [w * 100000 for w in weights])  # Default to percentage of $100k
            ...
            # Create table data
            table_data = []
            for ticker, weight, amount, shares in zip(filtered_tickers, filtered_weights, filtered_amounts, filtered_shares):
