...
class BaseAnalysisService:
    ...
    def _check_provider_async(self):
        ...
        self.is_async = (
            hasattr(self.provider, 'get_ticker_info') and 
    ...
    def _verify_sync_provider(self, method_name: str):
        ...
        """
        if self.is_async:
            raise TypeError(f"Cannot use sync method with async provider. Use {method_name} instead.")
    ...
    def _verify_async_provider(self, method_name: str):
        ...
        if not self.is_async:
            raise TypeError(f"Cannot use async method with sync provider. Use {method_name} instead.")