...
BENCHMARK_SETTINGS = PERFORMANCE_CONFIG.get("BENCHMARK", DEFAULT_BENCHMARK_CONFIG) if hasattr(PERFORMANCE_CONFIG, "BENCHMARK") else DEFAULT_BENCHMARK_CONFIG
...
class PerformanceBenchmark:
    ...
    def stop(self) -> Dict[str, Any]:
        ...
        if PERFORMANCE_CONFIG.get("ENABLE_MEMORY_PROFILING", False) and hasattr(self, '_tracemalloc_snapshot_start'):
    ...
    def print_summary(self) -> None:
        ...
        if self.results["iterations"]:
            ...
            for i, iteration in enumerate(self.results["iterations"]):
                print(f"[{i+1}] {iteration['name']}: {iteration['metrics'].get('elapsed_time_ms', 0):.2f} ms")
    ...
    def decorator(func):
        ...
        def sync_wrapper(*args, **kwargs):
            ...
            for i in range(iterations):
                ...
                bench.record_iteration(f"Iteration {i+1}", {
...
class ResourceMonitor:
    ...
    def print_summary(self) -> None:
        ...
        if "memory" in summary:
            mem = summary["memory"]
            print(f"Memory: min={mem['min_mb']:.2f}MB, max={mem['max_mb']:.2f}MB, avg={mem['avg_mb']:.2f}MB")
            print(f"Memory growth: {mem['growth_mb']:.2f}MB")
        
        if "cpu" in summary:
            cpu = summary["cpu"]
            print(f"CPU: min={cpu['min_percent']:.2f}%, max={cpu['max_percent']:.2f}%, avg={cpu['avg_percent']:.2f}%")
        
        if "threads" in summary:
            threads = summary["threads"]
            print(f"Threads: min={threads['min_count']}, max={threads['max_count']}, growth={threads['growth']}")
...
async def run_batch_size_benchmark(
    ...
    test_tickers = ["AAPL", "MSFT", "GOOG", "AMZN", "META", 
                   "TSLA", "NVDA", "JPM", "V", "JNJ", 
                   "WMT", "PG", "HD", "COST", "UNH",
                   "BAC", "DIS", "NFLX", "INTC", "AMD",
                   "CSCO", "ORCL", "IBM", "CRM", "ADBE"]
...
class BenchmarkResult:
    ...
    def as_dict(self) -> Dict[str, Any]:
        ...
        if self.memory_before > 0:
            ...
            result["memory_change_percent"] = ((self.memory_after / self.memory_before) - 1) * 100 if self.memory_before else 0
    ...
    def print_summary(self) -> None:
        ...
        if self.memory_before > 0:
            ...
            memory_change_pct = ((self.memory_after / self.memory_before) - 1) * 100 if self.memory_before else 0
    ...
    async def _refill_tokens(self) -> None:
        ...
        if elapsed > 0:
            ...
            self.tokens["MEDIUM"] = min(
                ...
                self.tokens["MEDIUM"] + (medium_refill_rate * elapsed)
            )
            
            self.tokens["LOW"] = min(
                self.low_priority_quota,
                self.tokens["LOW"] + (low_refill_rate * elapsed)
            )
    ...
    for i in range(0, len(items_queue), batch_size):
        ...
        if total_processed % performance_monitor_interval == 0:
            ...
            if success_rate > 0.95 and current_concurrency < max_concurrency:
                ...
                logger.debug(f"Increased concurrency to {current_concurrency} (success rate: {success_rate:.2f})")
            ...
            # Decrease concurrency if success rate is low
                ...
                logger.debug(f"Decreased concurrency to {current_concurrency} (success rate: {success_rate:.2f})")
        ...
        async def process_with_rate_limit(item: T) -> R:
            try:
                ...
                await rate_limiter.wait(
                    ticker=str(item) if hasattr(item, "__str__") else None,
                    priority=priority
                )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run performance benchmarks for the yahoofinance package")
    parser.add_argument("--provider", choices=["yahoo", "yahooquery", "hybrid", "all"], default="all",
                       help="Provider to benchmark (default: all)")
    parser.add_argument("--ticker-count", type=int, default=5,
    ...
    parser.add_argument("--concurrency-benchmark", action="store_true",
    ...
    parser.add_argument("--memory-check", action="store_true",
                       help="Run memory leak detection tests")
    parser.add_argument("--priority-test", action="store_true",
                       help="Test priority-based rate limiting")
    ...
    if args.memory_check:
        ...
        async def test_memory():
            ...
            is_leaking, stats = await find_memory_leaks_async(
                provider.batch_get_ticker_info, ["AAPL", "MSFT", "GOOG", "AMZN", "META"], iterations=5
            )
        ...
        asyncio.run(benchmark_provider(args.provider, ticker_count=args.ticker_count, iterations=args.iterations))
