...
YahooFinanceProvider = LazyImport(
    "yahoofinance.api.providers.yahoo_finance", "YahooFinanceProvider"
)
...
class ExampleEnhancedProvider(FinanceDataProvider):
    ...
    def __init__(
        self, max_retries: int = 3, retry_delay: float = 1.0, enable_circuit_breaker: bool = True
    ):
        """
        ...
        logger.debug(
            f"Initialized ExampleEnhancedProvider with max_retries={max_retries}, "
    ...
    def _get_context_for_ticker_op(self, ticker: str, operation: str, **kwargs) -> Dict[str, Any]:
        ...
        """
        return {"ticker": ticker, "operation": operation, "timestamp": time.time(), **kwargs}
    ...
    def get_optional_metadata(self, ticker: str) -> Dict[str, Any]:
        ...
        return self.base_provider.get_insider_data(ticker)
