...
class AsyncYahooQueryProvider(YahooFinanceBaseProvider, AsyncFinanceDataProvider):
    ...
    def __init__(self, max_retries: int = 3, retry_delay: float = 1.0):
        ...
        self._sync_provider = YahooQueryProvider(max_retries=max_retries, retry_delay=retry_delay)
    ...
    async def get_ticker_info(self, ticker: str, skip_insider_metrics: bool = False) -> Dict[str, Any]:
        ...
        return await self._run_in_executor(
            self._sync_provider.get_ticker_info,
            ticker,
            skip_insider_metrics
        )
    ...
    async def get_price_data(self, ticker: str) -> Dict[str, Any]:
        ...
        # Run the synchronous version in an executor to make it non-blocking
        return await self._run_in_executor(
            self._sync_provider.get_price_data,
            ticker
        )
    ...
    async def get_historical_data(self, ticker: str, period: str = "1y", interval: str = "1d") -> pd.DataFrame:
        ...
        # Run the synchronous version in an executor to make it non-blocking
        return await self._run_in_executor(
            self._sync_provider.get_historical_data,
            ticker,
            period,
            interval
        )
    ...
    async def get_earnings_dates(self, ticker: str) -> Tuple[Optional[str], Optional[str]]:
        ...
        # Run the synchronous version in an executor to make it non-blocking
        return await self._run_in_executor(
            self._sync_provider.get_earnings_dates,
            ticker
        )
    ...
    async def get_analyst_ratings(self, ticker: str) -> Dict[str, Any]:
        ...
        # Run the synchronous version in an executor to make it non-blocking
        return await self._run_in_executor(
            self._sync_provider.get_analyst_ratings,
            ticker
        )
    ...
    async def get_insider_transactions(self, ticker: str) -> List[Dict[str, Any]]:
        ...
        # Run the synchronous version in an executor to make it non-blocking
        return await self._run_in_executor(
            self._sync_provider.get_insider_transactions,
            ticker
        )
    ...
    async def search_tickers(self, query: str, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        # Run the synchronous version in an executor to make it non-blocking
        return await self._run_in_executor(
            self._sync_provider.search_tickers,
            query,
            limit
        )
    ...
    async def batch_get_ticker_info(self, tickers: List[str], skip_insider_metrics: bool = False) -> Dict[str, Dict[str, Any]]:
        ...
        return await self._run_in_executor(
            self._sync_provider.batch_get_ticker_info,
            tickers,
            skip_insider_metrics
        )