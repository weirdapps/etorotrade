...
from ...utils.market.ticker_utils import (  # Go up two levels (providers -> api -> yahoofinance) then down
    validate_ticker,
)
...
class OptimizedAsyncYFinanceProvider(AsyncFinanceDataProvider):
    ...
    def __init__(self):
        ...
        self.cache_manager = CacheManager(
            ...
            memory_cache_size=CACHE_CONFIG.get(
                "MEMORY_CACHE_SIZE"
            ),  # Let constructor use default from config if None
            memory_cache_ttl=CACHE_CONFIG.get(
                "TICKER_INFO_MEMORY_TTL"
            ),  # Use specific TTL for this provider's data
            disk_cache_dir=CACHE_CONFIG.get("DISK_CACHE_DIR"),
            disk_cache_size_mb=CACHE_CONFIG.get("DISK_CACHE_SIZE_MB"),
            disk_cache_ttl=CACHE_CONFIG.get(
                "TICKER_INFO_DISK_TTL"
            ),  # Use specific TTL for this provider's data
        )
        self._stock_cache = {}  # Cache for yfinance Ticker objects (kept simple for now)
        self._ratings_cache = (
            {}
        )  # Cache for post-earnings ratings calculations (specific internal logic)
    ...
    async def _check_rate_limit(self):
        ...
        self._rate_limiter["call_timestamps"] = [
            ts
            for ts in self._rate_limiter["call_timestamps"]
            if now - ts <= self._rate_limiter["window_size"]
        ]
        # Check for recent errors
        if (
            self._rate_limiter["last_error_time"] > 0
            and now - self._rate_limiter["last_error_time"] < 120
        ):
            extra_wait = 5.0
            logger.warning(
                f"Recent rate limit error detected. Adding {extra_wait}s additional delay."
            )
    ...
    async def get_ticker_info(
        ...
        try:
            ...
            info = {
                ...
                "company": ticker_info.get("longName", ticker_info.get("shortName", ""))[
                    :14
                ].upper(),
                ...
                "dividend_yield": (
                    ...
                    if ticker_info.get("dividendYield", None) is not None
                    else None
                ),
                ...
                "short_percent": (
                    ticker_info.get("shortPercentOfFloat", None) * 100
                    if ticker_info.get("shortPercentOfFloat", None) is not None
                    else None
                ),
            ...
            if ticker_info.get("numberOfAnalystOpinions", 0) > 0:
                try:
                    ...
                    if recommendations is not None and not recommendations.empty:
                        ...
                        # No action needed for fallback in this simplified example
                        # ... (original fallback logic based on rec_key) ...
                        info["buy_percentage"] = 50  # Simplified example
                        info["total_ratings"] = ticker_info.get("numberOfAnalystOpinions", 0)
                except YFinanceError as e:  # Fallback
                    logger.debug(
                        f"Error getting recommendations for {ticker}: {e}, falling back to recommendationKey"
                    )
                    ticker_info.get("recommendationKey", "").lower()
                    # ... (original fallback logic based on rec_key) ...
                    info["buy_percentage"] = 50  # Simplified example
                    info["total_ratings"] = ticker_info.get("numberOfAnalystOpinions", 0)
            else:
    ...
    async def get_price_data(self, ticker: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "fifty_two_week_high": info.get(
                "fifty_two_week_high"
            ),  # Ensure these are fetched in get_ticker_info if needed
    ...
    def _has_post_earnings_ratings(self, ticker: str, yticker) -> bool:
        ...
        try:
            ...
            try:
                ...
                if (
                    "GradeDate" not in df.columns
                    and hasattr(upgrades_downgrades, "index")
                    and isinstance(upgrades_downgrades.index, pd.DatetimeIndex)
                ):
                    pass
    ...
    def _get_last_earnings_date(self, yticker):
        ...
        try:  # Try calendar first
            ...
            if isinstance(calendar, dict) and COLUMN_NAMES["EARNINGS_DATE"] in calendar:
                ...
                if isinstance(earnings_date_list, list) and len(earnings_date_list) > 0:
                    ...
                    past_earnings = [
                        d for d in earnings_date_list if isinstance(d, datetime.date) and d < today
                    ]  # Ensure date objects
                    if past_earnings:
        ...
        try:  # Try earnings_dates attribute
            ...
            if earnings_dates is not None and not earnings_dates.empty:
                ...
                if tz:
                    today = today.tz_localize(tz)  # Match timezone if index has one
        ...
        try:  # Add try-except for safety
            ...
            if val >= 1e12:
                ...
                return (
                    f"{int(val / 1e9)}B"
                    if val >= 100e9
                    else (f"{val / 1e9:.1f}B" if val >= 10e9 else f"{val / 1e9:.2f}B")
                )
            elif val >= 1e6:
                return (
                    f"{int(val / 1e6)}M"
                    if val >= 100e6
                    else (f"{val / 1e6:.1f}M" if val >= 10e6 else f"{val / 1e6:.2f}M")
                )
            else:
                return f"{int(val):,}"
        except (ValueError, TypeError):
