...
from yahoofinance.utils.market.ticker_utils import is_us_ticker, validate_ticker
...
class CustomYahooFinanceProvider(AsyncFinanceDataProvider):
    ...
    async def _process_buy_percentage(
        self, ticker: str, yticker, ticker_info: Dict[str, Any], info: Dict[str, Any]
    ):
            ...
            try:
                ...
                if recommendations is not None and not recommendations.empty:
                    ...
                    if total > 0:
                        ...
                        logger.debug(
                            f"Using recommendations data for {ticker}: {buy_count}/{total} = {buy_percentage:.1f}%"
                        )
                    else:
                ...
                logger.warning(
                    f"Unexpected error getting recommendations for {ticker}: {e}, falling back to recommendationKey"
                )
                self._process_recommendation_key(ticker_info, info)
        else:
    ...
    async def _process_earnings_ratings(self, ticker: str, yticker, info: Dict[str, Any]):
        ...
        if self._is_us_ticker(ticker) and info.get("total_ratings", 0) > 0:
            ...
            if has_post_earnings and ticker in self._ratings_cache:
                ...
                logger.debug(
                    f"Using post-earnings ratings for {ticker}: buy_pct={ratings_data['buy_percentage']:.1f}%, total={ratings_data['total_ratings']}"
                )
    ...
    async def _process_earnings_date(self, ticker: str, yticker, info: Dict[str, Any]):
        ...
        try:
            ...
            try:
                if (
                    hasattr(yticker, "earnings_dates")
                    and yticker.earnings_dates is not None
                    and not yticker.earnings_dates.empty
                ):
                    # Get now in the same timezone as earnings dates
                    today = pd.Timestamp.now()
                    if (
                        hasattr(yticker.earnings_dates.index, "tz")
                        and yticker.earnings_dates.index.tz is not None
                    ):
    ...
    async def get_earnings_data(self, ticker: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                next_earnings = (
                    yticker.earnings_dates.head(1) if hasattr(yticker, "earnings_dates") else None
                )
    ...
    def _process_error_for_batch(self, ticker: str, error: Exception) -> Dict[str, Any]:
        ...
        return {"symbol": ticker, "error": str(error), "ticker": ticker, "company": ticker}
