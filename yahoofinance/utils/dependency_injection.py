...
import importlib
import inspect
from typing import Dict, Any, Callable, Type, Optional, List, Union, TypeVar, Generic, cast
...
class Registry:
    ...
    def register(self, key: str, factory: Optional[Callable[..., T]] = None) -> Union[Callable[..., T], Callable[[Callable[..., T]], Callable[..., T]]]:
        """
    ...
    try:
        ...
        def _lazy_import():
            try:
                ...
                logger.error(f"Lazy import failed: {module_path}.{class_name if class_name else ''} - {str(ex)}")
                raise ImportError(f"Failed to import {module_path}.{class_name if class_name else ''}") from ex
