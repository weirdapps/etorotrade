...
class RateLimiter:
    ...
    def __init__(
        ...
        self.ticker_priority_multipliers = RATE_LIMIT.get("TICKER_PRIORITY", {
        ...
        self.cache_aware = cache_aware if cache_aware is not None else RATE_LIMIT.get("CACHE_AWARE_RATE_LIMITING", False)
        ...
        logger.debug(f"Initialized enhanced rate limiter with window_size={self.window_size}, "
    ...
    def _start_adaptive_monitoring(self) -> None:
        ...
        monitoring_thread = threading.Thread(
            target=monitor_and_adapt, 
            daemon=True,
            name="RateLimiterAdaptiveMonitoring"
        )
    ...
    def _adapt_rate_limiting_strategy(self) -> None:
        ...
        with self.lock:
            ...
            success_rate = self.metrics["successful_calls"] / total_calls if total_calls > 0 else 1.0
            ...
            if rate_limit_rate > 0.01:  # More than 1% rate limit errors
                ...
                logger.warning(f"Increasing delay to {self.delay:.3f}s due to {rate_limit_rate:.1%} rate limit errors")
            elif error_rate > self.max_error_rate:
                # Too many errors, increase delay
                self.delay = min(self.delay * 1.2, self.max_delay)
                logger.warning(f"Increasing delay to {self.delay:.3f}s due to high error rate ({error_rate:.1%})")
                ...
                logger.warning(f"Increasing delay to {self.delay:.3f}s due to low success rate ({success_rate:.1%})")
                ...
                logger.info(f"Decreasing delay to {self.delay:.3f}s due to excellent success rate ({success_rate:.1%})")
            
            # Log adaptation if delay changed
            if abs(self.delay - old_delay) > 0.001:
                logger.info(f"Adapted rate limiting strategy: {old_delay:.3f}s -> {self.delay:.3f}s | "
            ...
            with self.lock:
                if self.call_timestamps:
                    ...
                    if window_wait_time > 0:
                        logger.warning(f"Rate limit would be exceeded. Waiting {window_wait_time:.2f} seconds.")
    ...
    def record_success(self, ticker: Optional[str] = None) -> None:
        ...
        with self.lock:
            ...
            if self.success_streak >= self.success_threshold:
                ...
                logger.debug(f"Decreased delay to {self.delay:.3f}s after {self.success_threshold} consecutive successes")
    ...
    def record_cache_hit(self, ticker: Optional[str] = None) -> None:
        ...
        with self.lock:
            ...
            if ticker is not None:
                ...
                if current_hits >= 10 and ticker not in self.vip_tickers and ticker not in self.slow_tickers:
            ...
            if self.cache_hit_streak >= self.cache_hit_streak_threshold:
                ...
                reduced_delay = max(self.delay * self.cache_hit_delay_reduction, self.cache_hit_min_delay)
                
                # Only log if there's a significant change
                if abs(reduced_delay - self.delay) > 0.01:
                    logger.debug(f"Decreased global delay from {self.delay:.3f} to {reduced_delay:.3f} after {self.cache_hit_streak} consecutive cache hits")
        ...
        with self.lock:
            ...
            if ticker is not None:
                ...
                if ticker in self.vip_tickers:
                    ...
                    multiplier = self.rate_limit_delay_increase if is_rate_limit else self.error_delay_increase
                ...
                logger.debug(f"Set ticker-specific delay for {ticker} to {new_delay:.3f}s (multiplier: {multiplier})")
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            avg_response = self.metrics["total_response_time"] / total_calls if total_calls > 0 else 0
            ...
            return {
                ...
                "unique_tickers": len(set(self.ticker_priorities.keys()) | 
    ...
    def decorator(f: Callable[..., T]) -> Callable[..., T]:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> T:
            ...
            try:
                ...
                if cache_aware and limiter.cache_aware and isinstance(result, dict) and 'from_cache' in result:
                ...
                logger.error(f"Unexpected error in rate limited function: {type(e).__name__}: {str(e)}")
