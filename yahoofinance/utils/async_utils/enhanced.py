...
class AsyncRateLimiter:
    ...
    def __init__(self, 
        ...
        self.ticker_priority = RATE_LIMIT.get("TICKER_PRIORITY", {
        ...
        async with self.lock:
            ...
            if len(self.call_times) >= self.max_calls:
                ...
                if wait_time > 0:
                    ...
                    logger.debug(f"Rate limit reached. Waiting {wait_time:.2f}s for ticker: {ticker}")
            ...
            if self.enable_adaptive_strategy and (now - self.last_strategy_update) > self.monitor_interval:
    ...
    async def record_success(self, ticker: Optional[str] = None) -> None:
        ...
        async with self.lock:
            ...
            if self.success_streak >= self.success_threshold:
                self.current_delay = max(self.min_delay, self.current_delay * self.success_delay_reduction)
                logger.debug(f"Reduced delay to {self.current_delay:.2f}s after {self.success_streak} successes")
    
    async def record_failure(self, is_rate_limit: bool = False, ticker: Optional[str] = None) -> None:
        """
        ...
        async with self.lock:
            ...
            if is_rate_limit:
                ...
                self.current_delay = min(self.max_delay, self.current_delay * self.rate_limit_delay_increase)
                logger.warning(f"Rate limit detected for ticker {ticker}. Increased delay to {self.current_delay:.2f}s")
            elif self.failure_streak >= self.error_threshold:
                # Increase delay after consecutive failures
                self.current_delay = min(self.max_delay, self.current_delay * self.error_delay_increase)
                logger.debug(f"Increased delay to {self.current_delay:.2f}s after {self.failure_streak} failures")
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        with asyncio.Lock():
            ...
            metrics = {
                ...
                "min_wait_time": round(self.min_wait_time, 3) if self.min_wait_time != float('inf') else 0,
    ...
    def _get_delay_for_ticker(self, ticker: Optional[str] = None) -> float:
        ...
        if ticker.endswith(".L") or ticker.endswith(".PA") or ticker.endswith(".DE") or ticker.endswith(".MI"):
            delay *= self.europe_delay_multiplier
        elif ticker.endswith(".T") or ticker.endswith(".HK") or ticker.endswith(".SS") or ticker.endswith(".SZ"):
    ...
    async def _update_adaptive_strategy(self) -> None:
        ...
        if self.total_calls > 0:
            ...
            if error_rate > self.max_error_rate:
                ...
                logger.info(f"Adaptive strategy: Error rate {error_rate:.1%} exceeds threshold. "
                ...
                logger.info(f"Adaptive strategy: Success rate {success_rate:.1%} exceeds threshold. "
...
class PriorityAsyncRateLimiter(AsyncRateLimiter):
    ...
    def __init__(self,
        ...
        logger.debug(f"Initialized PriorityAsyncRateLimiter with quotas: "
    ...
    async def wait(self, ticker: Optional[str] = None) -> float:
        ...
        async with self.lock:
            ...
            for p in self.priority_call_times:
                self.priority_call_times[p] = [t for t in self.priority_call_times[p] if t >= window_start]
            ...
            if len(self.call_times) >= self.max_calls:
                ...
                if wait_time > 0:
                    self.rate_limited_calls += 1
                    logger.debug(f"Global rate limit reached. Waiting {wait_time:.2f}s for ticker: {ticker} (priority: {priority})")
                    self.lock.release()
                    try:
                        await asyncio.sleep(wait_time)
                    finally:
            ...
            if len(self.priority_call_times[priority]) >= priority_quota:
                ...
                if priority_wait_time > 0:
                    logger.debug(f"{priority} priority quota reached. Waiting {priority_wait_time:.2f}s for ticker: {ticker}")
                    self.lock.release()
                    try:
            ...
            if additional_delay > 0:
                if additional_delay > 0.05:  # Only log if waiting more than 50ms
                    logger.debug(f"Adding delay of {additional_delay:.2f}s for ticker: {ticker} (priority: {priority})")
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        for priority in self.priority_metrics:
            ...
            priority_stats[priority] = {
                ...
                "utilization": round(len(self.priority_call_times[priority]) / {
                    "HIGH": self.high_priority_quota,
                    "MEDIUM": self.medium_priority_quota,
                    "LOW": self.low_priority_quota
                }[priority] * 100, 1)
