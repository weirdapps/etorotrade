...
def check_confidence_criteria(row, criteria):
    ...
    if not (ANALYST_COUNT in row or TOTAL_RATINGS in row) and ("# T" in row or "# A" in row):
        ...
        column_mapping = {"# T": ANALYST_COUNT, "# A": TOTAL_RATINGS}
    ...
    if not has_enough_coverage:
        ...
        logger.debug(
            f"Stock {ticker} fails confidence check: analysts={analyst_count}/{min_analysts}, targets={total_ratings}/{min_targets}"
        )
    ...
    if UPSIDE in row and pd.notna(row[UPSIDE]):
        ...
        try:
            ...
            if upside_value < sell_criteria["SELL_MAX_UPSIDE"]:
                return (
                    True,
                    f"Low upside ({upside_value:.1f}% < {sell_criteria['SELL_MAX_UPSIDE']}%)",
                )
        except (ValueError, TypeError):
    ...
    if BUY_PERCENTAGE_COL in row and pd.notna(row[BUY_PERCENTAGE_COL]):
        ...
        try:
            if isinstance(row[BUY_PERCENTAGE_COL], str):
                # Skip comparison for '--' values or other non-numeric strings
                if (
                    row[BUY_PERCENTAGE_COL] == "--"
                    or not row[BUY_PERCENTAGE_COL].replace(".", "", 1).replace("%", "", 1).isdigit()
                ):
                    return False, None
            ...
            if buy_pct < sell_criteria["SELL_MIN_BUY_PERCENTAGE"]:
                return (
                    True,
                    f"Low buy percentage ({buy_pct:.1f}% < {sell_criteria['SELL_MIN_BUY_PERCENTAGE']}%)",
                )
        except (ValueError, TypeError):
            # If conversion fails, skip this criterion
            return False, None
...
def _check_pe_ratio_sell_criterion(row):
    ...
    if (
        PE_FORWARD in row
        and PE_TRAILING in row
        and pd.notna(row[PE_FORWARD])
        and pd.notna(row[PE_TRAILING])
    ):
        # Safe conversion handling special values
        try:
            # Handle PE_FORWARD
            if isinstance(row[PE_FORWARD], str):
                ...
                if (
                    row[PE_FORWARD] == "--"
                    or not row[PE_FORWARD].replace(".", "", 1).replace("-", "", 1).isdigit()
                ):
                    return False, None
            ...
            if isinstance(row[PE_TRAILING], str):
                ...
                if (
                    row[PE_TRAILING] == "--"
                    or not row[PE_TRAILING].replace(".", "", 1).replace("-", "", 1).isdigit()
                ):
                    return False, None
            ...
            if pe_forward > 0 and pe_trailing > 0 and (pe_forward - pe_trailing) > 0.5:
                return (
                    True,
                    f"Worsening P/E ratio (Forward - Trailing > 0.5: {pe_forward:.1f} - {pe_trailing:.1f} = {(pe_forward - pe_trailing):.1f})",
                )
        except (ValueError, TypeError):
            # If conversion fails, skip this criterion
            return False, None
    ...
    if "pe_forward" in row and pd.notna(row["pe_forward"]):
        ...
        try:
            ...
            if pe_forward < 0:
                ...
                return (
                    True,
                    f"High forward P/E ({pe_forward:.1f} > {sell_criteria['SELL_MIN_FORWARD_PE']})",
                )
        except (ValueError, TypeError):
            # If conversion fails, skip this criterion
            return False, None
    ...
    if "peg_ratio" in row and pd.notna(row["peg_ratio"]):
        ...
        try:
            if isinstance(row["peg_ratio"], str):
                ...
                if (
                    row["peg_ratio"] == "--"
                    or not row["peg_ratio"].replace(".", "", 1).replace("-", "", 1).isdigit()
                ):
                    return False, None
    ...
    if short_field in row and pd.notna(row[short_field]):
        ...
        try:
            if isinstance(row[short_field], str):
                ...
                if (
                    row[short_field] == "--"
                    or not row[short_field].replace(".", "", 1).replace("%", "", 1).isdigit()
                ):
                    return False, None
            ...
            if short_value > sell_criteria["SELL_MIN_SHORT_INTEREST"]:
                return (
                    True,
                    f"High short interest ({short_value:.1f}% > {sell_criteria['SELL_MIN_SHORT_INTEREST']}%)",
                )
        except (ValueError, TypeError):
            # If conversion fails, skip this criterion
            return False, None
    ...
    if "beta" in row and pd.notna(row["beta"]):
        ...
        try:
            if isinstance(row["beta"], str):
                ...
                if (
                    row["beta"] == "--"
                    or not row["beta"].replace(".", "", 1).replace("-", "", 1).isdigit()
                ):
                    return False, None
    ...
    if EXRET in row and pd.notna(row[EXRET]):
        ...
        try:
            ...
            if exret_value < sell_criteria["SELL_MAX_EXRET"]:
                return (
                    True,
                    f"Low expected return ({exret_value:.1f}% < {sell_criteria['SELL_MAX_EXRET']}%)",
                )
        except (ValueError, TypeError):
            # If conversion fails, skip this criterion
            return False, None
...
def _check_upside_buy_criterion(row, buy_criteria):
    ...
    try:
        ...
        if upside_value < buy_criteria["BUY_MIN_UPSIDE"]:
            return (
                False,
                f"Insufficient upside ({upside_value:.1f}% < {buy_criteria['BUY_MIN_UPSIDE']}%)",
            )
    except (ValueError, TypeError):
        # If conversion fails, skip this criterion
...
def _check_buy_percentage_buy_criterion(row, buy_criteria):
    ...
    try:
        ...
        if buy_pct < buy_criteria["BUY_MIN_BUY_PERCENTAGE"]:
            return (
                False,
                f"Insufficient buy percentage ({buy_pct:.1f}% < {buy_criteria['BUY_MIN_BUY_PERCENTAGE']}%)",
            )
    except (ValueError, TypeError):
        # If conversion fails, skip this criterion
    ...
    if "peg_ratio" in row and pd.notna(row["peg_ratio"]):
        ...
        try:
            if isinstance(row["peg_ratio"], str):
                ...
                if (
                    ...
                    return (
                        True,
                        None,
                    )  # Not a numeric value, so we can't compare - secondary criterion
                peg_value = float(row["peg_ratio"])
            else:
                peg_value = float(row["peg_ratio"])
            ...
            if peg_value >= buy_criteria["BUY_MAX_PEG"]:
                return (
                    False,
                    f"PEG ratio too high ({peg_value:.1f} ≥ {buy_criteria['BUY_MAX_PEG']})",
                )
        except (ValueError, TypeError):
    ...
    if short_field in row and pd.notna(row[short_field]):
        ...
        try:
            ...
            if short_value > buy_criteria["BUY_MAX_SHORT_INTEREST"]:
                return (
                    False,
                    f"Short interest too high ({short_value:.1f}% > {buy_criteria['BUY_MAX_SHORT_INTEREST']}%)",
                )
        except (ValueError, TypeError):
            # If conversion fails, skip this criterion (it's a secondary criterion)
            return True, None
    ...
    if EXRET in row and pd.notna(row[EXRET]):
        ...
        try:
            ...
            if exret_value < buy_criteria.get("BUY_MIN_EXRET", 0):
                return (
                    False,
                    f"Expected return too low ({exret_value:.1f}% < {buy_criteria.get('BUY_MIN_EXRET', 0)}%)",
                )
        except (ValueError, TypeError):
            # If conversion fails, skip this criterion
...
def _is_forward_pe_in_range(row, criteria):
    ...
    try:
        ...
        pe_forward = (
            float(row["pe_forward"]) if isinstance(row["pe_forward"], str) else row["pe_forward"]
        )
        min_forward_pe = (
            float(criteria["BUY_MIN_FORWARD_PE"])
            if isinstance(criteria["BUY_MIN_FORWARD_PE"], str)
            else criteria["BUY_MIN_FORWARD_PE"]
        )
        max_forward_pe = (
            float(criteria["BUY_MAX_FORWARD_PE"])
            if isinstance(criteria["BUY_MAX_FORWARD_PE"], str)
            else criteria["BUY_MAX_FORWARD_PE"]
        )

        return pe_forward > min_forward_pe and pe_forward <= max_forward_pe
    except (ValueError, TypeError):
...
def _is_pe_improving(row):
    ...
    try:
        # Handle string values - convert to float first
        pe_trailing = (
            float(row["pe_trailing"]) if isinstance(row["pe_trailing"], str) else row["pe_trailing"]
        )
        pe_forward = (
            float(row["pe_forward"]) if isinstance(row["pe_forward"], str) else row["pe_forward"]
        )
...
def calculate_action_for_row(row, criteria, short_field=DEFAULT_SHORT_FIELD):
    ...
    ticker = (
        normalized_row.get("ticker", "")
        or normalized_row.get("TICKER", "")
        or row.get("TICKER", "unknown")
    )
...
def normalize_row_columns(row, column_mapping=None):
    ...
    for internal_col in internal_cols:
        if internal_col in row and internal_col not in normalized_row:
            ...
            if isinstance(value, str) and internal_col in [
                "beta",
                "pe_trailing",
                "pe_forward",
                "peg_ratio",
    ...
    if (
        ("EXRET" not in normalized_row or normalized_row["EXRET"] is None)
        and UPSIDE in normalized_row
        and BUY_PERCENTAGE_COL in normalized_row
    ):
