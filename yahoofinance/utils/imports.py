...
def import_module_or_object(
    module_path: str, object_name: Optional[str] = None
) -> Any:
    """
    ...
    try:
        ...
        raise ImportError(
            f"Failed to import {object_name or module_path}: {str(e)}"
        ) from e
...
def local_import(name: str) -> Any:
    ...
    if package_parts[0] != 'yahoofinance':
        raise ImportError(f"local_import can only be used within yahoofinance package, not {caller_module}")
