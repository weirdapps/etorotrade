...
from .monitoring import (
    GaugeMetric, HealthCheck, HealthStatus, MetricType,
    health_monitor, metrics_registry, monitor_function
)
...
trade_metrics = {
    "trades_executed": metrics_registry.counter(
        "business_trades_executed",
        "Number of trades executed"
    ),
    "buy_trades": metrics_registry.counter(
        "business_buy_trades",
        "Number of buy trades executed"
    ),
    "sell_trades": metrics_registry.counter(
        "business_sell_trades",
        "Number of sell trades executed"
    ),
    "hold_recommendations": metrics_registry.counter(
        "business_hold_recommendations",
        "Number of hold recommendations generated"
    ),
    "buy_recommendations": metrics_registry.counter(
        "business_buy_recommendations",
        "Number of buy recommendations generated"
    ),
    "sell_recommendations": metrics_registry.counter(
        "business_sell_recommendations",
        "Number of sell recommendations generated"
    ),
    "trade_volume": metrics_registry.gauge(
        "business_trade_volume",
        "Total trade volume"
    ),
    "portfolio_value": metrics_registry.gauge(
...
resource_metrics = {
    "cpu_percent": metrics_registry.gauge(
        "system_cpu_percent",
    ...
    "memory_vms": metrics_registry.gauge(
        "system_memory_vms_bytes",
    ...
    "open_files": metrics_registry.gauge(
        "system_open_files",
    ...
    "python_objects": metrics_registry.gauge(
        "system_python_objects",
...
data_metrics = {
    ...
    "api_retries": metrics_registry.counter(
        "data_api_retries",
        "Number of API retries performed"
    ),
    "batch_size": metrics_registry.gauge(
        "data_batch_size",
        "Size of data processing batches"
    ),
}
...
network_metrics = {
    ...
    "http_errors": metrics_registry.counter(
        "network_http_errors",
        "Number of HTTP request errors"
    ),
    "http_timeouts": metrics_registry.counter(
        "network_http_timeouts",
        "Number of HTTP request timeouts"
    ),
    "http_retries": metrics_registry.counter(
        "network_http_retries",
    ...
    def check_system_resources() -> HealthCheck:
        ...
        try:
            ...
            if memory_percent > 90 or cpu_percent > 90:
                ...
                details = f"Critical resource usage: Memory={memory_percent:.1f}%, CPU={cpu_percent:.1f}%"
            elif memory_percent > 75 or cpu_percent > 75:
                status = HealthStatus.DEGRADED
                details = f"High resource usage: Memory={memory_percent:.1f}%, CPU={cpu_percent:.1f}%"
            else:
                status = HealthStatus.HEALTHY
                details = f"Normal resource usage: Memory={memory_percent:.1f}%, CPU={cpu_percent:.1f}%"
    ...
    for category, metrics_dict in categorized_metrics.items():
        if metrics_dict:
            ...
            for name, metric in sorted(metrics_dict.items()):
                ...
                elif metric.type.value == "histogram" and hasattr(metric, "values") and metric.values:
                    report.append(f"{name}: count={len(metric.values)}, avg={sum(metric.values)/len(metric.values):.2f}")
