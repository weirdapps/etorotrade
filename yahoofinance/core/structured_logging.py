"""
Structured logging implementation for the Yahoo Finance data access package.

This module provides utilities for structured JSON logging with consistent
context information, making it easier to aggregate and analyze logs in
...
from .logging_config import DEFAULT_FORMAT, get_logger
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        for key in self.standard_attrs:
            ...
            if key in ("levelname", "name") and (
                (key == "levelname" and self.include_level) or 
                (key == "name" and self.include_logger_name)
            ):
                continue
        ...
        def middleware(request: Any) -> Any:
            ...
            try:
                ...
                request_logger.error(
                    f"Request failed: {context['method']} {context['path']} - {e}",
                    exc_info=True
                )
