...
from .monitoring import (
    CircuitBreakerStatus, HealthStatus, alert_manager, circuit_breaker_monitor, 
    health_monitor, metrics_registry, monitoring_service, request_tracker
)
    ...
    def handle_health(self) -> None:
        ...
        try:
            ...
            if "api_errors_total" in metrics:
                ...
                requests = metrics["api_requests_total"].value if "api_requests_total" in metrics else 0
                if requests > 0:
                    response["error_rate"] = round((metrics["api_errors_total"].value / requests) * 100, 2)
            ...
            if metric_type == "counter" or metric_type == "gauge":
                ...
                if hasattr(metric, "values") and metric.values:
                    ...
                    for i, bucket in enumerate(metric.buckets):
                        ...
                        prometheus_output.append(f"{name}_bucket{tags_str} {metric.bucket_counts[i]}")
...
class HealthEndpointServer:
    ...
    def start(self) -> None:
        ...
        try:
            ...
            self.server = socketserver.ThreadingTCPServer((self.host, self.port), HealthEndpointHandler)
