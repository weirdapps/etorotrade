...
def main():
    ...
    print("=" * 80)
    print("Analysis Date: 2026-04-21")
    ...
    report = {
        ...
        "portfolio_news": {ticker: [] for ticker in TOP_15_TICKERS},
        "earnings_calendar": {"next_2_weeks": sorted(earnings_cal, key=lambda x: x["days_away"])},
        ...
        "event_risk_summary": {
            ...
            "reason": f"Earnings in {earnings_cal[0]['days_away']} days"
            if earnings_cal
            else "No imminent events",
    ...
    print(
        f"  - Earnings in next week: {report['event_risk_summary']['earnings_count_next_week']} events"
    )
    print(f"  - Analyst coverage analyzed for {len(coverage)} stocks")
    print("\nNext: Claude will enrich with WebSearch for:")
    print("  1. Breaking market news")
    print("  2. Portfolio-specific news (top 15)")
    print("  3. Economic calendar events")
    print("  4. Analyst rating waves")
    print("  5. Regulatory/M&A developments")
