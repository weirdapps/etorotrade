...
def get_portfolio_value():
    ...
    parser.add_argument(
        "-v", "--value", type=float, help="Portfolio value in thousands (e.g., 530 for $530,000)"
    )
...
def analyze_portfolio_geography(portfolio_size=None):
    ...
    df["asset_type"] = df.apply(
        lambda row: classify_asset_type(row["symbol"], row.get("exchangeName", "")), axis=1
    )
    ...
    geography_data = defaultdict(
        lambda: {"count": 0, "total_value": 0.0, "tickers": [], "positions": []}
    )
    ...
    sorted_regions_ordered = sorted(
        geography_data.items(), key=lambda x: x[1]["total_value"], reverse=True
    )
    ...
    for region, data in sorted_regions_ordered:
        ...
        print(
            f"{region:<15} {data['count']:^10} ${data['total_value']:>13,.0f} {percentage:>8.1f}%  {top_holdings}"
        )
