...
async def main():
    ...
    test_tickers = ["AAPL", "MSFT", "NVDA", "GOOGL", "META", "TSLA", "AMZN", "JPM", "BAC", "WMT", "TGT"]
    ...
    for ticker in test_tickers:
        ...
        try:
            ...
            if 'earningsDate' in info_dict:
                ...
                print(f"Raw earningsDate value: {raw_earnings_date} (type: {type(raw_earnings_date)})")
                
                # Try to format it
                if isinstance(raw_earnings_date, (int, float)):
                    # Convert from timestamp if needed
                    formatted_date = datetime.datetime.fromtimestamp(raw_earnings_date).strftime('%Y-%m-%d')
                    ...
                    formatted_dates = [datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d') 
