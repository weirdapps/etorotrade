...
from yahoofinance.utils.network.rate_limiter import (
    RateLimiter, rate_limited, global_rate_limiter
)
...
class TestRateLimiterThreadSafety:
    ...
    def test_lock_during_operations(self):
        ...
        assert mock_lock.__enter__.call_count >= 3, f"Only used lock {mock_lock.__enter__.call_count} times"
        assert mock_lock.__exit__.call_count >= 3, f"Only exited lock {mock_lock.__exit__.call_count} times"
...
class TestBatchProcessing:
    ...
    def test_batch_process_with_errors(self):
        ...
        with patch('yahoofinance.utils.network.batch.BatchProcessor._process_batch') as mock_process:
