    ...
    def test_apply_all_filters(self, sample_market_data, trading_criteria_config):
        ...
        if hasattr(filter_instance, "apply_filters"):
            ...
            if len(filtered_data) > 0:
                assert all(filtered_data["upside"] >= trading_criteria_config["min_upside"])
                assert all(
                    filtered_data["buy_percentage"] >= trading_criteria_config["min_buy_percentage"]
                )
                assert all(
                    filtered_data["analyst_count"] >= trading_criteria_config["min_analyst_count"]
                )
...
class TestFilterChaining:
    ...
    def test_criteria_and_portfolio_filter_chain(
        self, sample_market_data, sample_portfolio_data, trading_criteria_config
    ):
