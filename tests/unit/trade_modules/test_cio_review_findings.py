...
class TestConvictionSizer:
    ...
    CIO v36 / M2: legacy continuous-conviction curve disabled by default
    (CONVICTION_CLAMP_TO_UNITY=True). The autouse fixture below disables
    the clamp for this class so the legacy curve is testable for when
    the clamp is reverted (per per-cell ρ evidence).
    """
...
class TestPriceBasedRotation:
    ...
    def test_combined_rotation_both_agree(self):
        ...
        with patch("trade_modules.sector_rotation.detect_sector_rotation") as mock:
            ...
            result = detect_combined_rotation(sector_returns=sector_returns)
...
class TestEarningsProximity:
    ...
    def test_earnings_imminent(self):
        ...
        with patch(
            "trade_modules.earnings_proximity._fetch_next_earnings", return_value=future_date
        ):
...
class TestStalenessPenalties:
    ...
    def test_thresholds_correct(self):
        ...
        from trade_modules.data_freshness import AGING_THRESHOLD, FRESH_THRESHOLD, STALE_THRESHOLD
