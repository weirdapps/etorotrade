...
class TestAnalyzeMarketOpportunities:
    ...
    @pytest.mark.asyncio
    async def test_analyze_with_existing_bs_column(
        self, trading_engine, sample_market_data_with_bs
    ):
    ...
    @pytest.mark.asyncio
    async def test_analyze_without_signals_calls_calculation(
        self, trading_engine, sample_market_data_no_signals
    ):
        """Test analysis when no BS/ACT column exists - should calculate signals."""
        # Mock the signal calculation methods
        with patch.object(
            trading_engine.analysis_service, "calculate_trading_signals"
        ) as mock_calc:
        ...
        assert all(
            key in result
            for key in ["buy_opportunities", "sell_opportunities", "hold_opportunities"]
        )

    @pytest.mark.asyncio
    async def test_analyze_with_portfolio_filtering(
        self, trading_engine, sample_market_data_with_bs, sample_portfolio_data
    ):
        """Test analysis with portfolio filtering applied."""
        result = await trading_engine.analyze_market_opportunities(
            sample_market_data_with_bs, portfolio_df=sample_portfolio_data
        )
    ...
    @pytest.mark.asyncio
    async def test_analyze_with_notrade_filtering(
        self, trading_engine, sample_market_data_with_bs, temp_notrade_file
    ):
        """Test analysis with notrade filtering."""
        result = await trading_engine.analyze_market_opportunities(
            sample_market_data_with_bs, notrade_path=temp_notrade_file
        )
    ...
    async def test_analyze_logs_results_summary(self, trading_engine, sample_market_data_with_bs):
        ...
        summary_calls = [
            call for call in mock_logger.call_args_list if "Analysis complete" in str(call)
        ]
...
class TestFilteringMethods:
    ...
    def test_filter_sell_opportunities_with_confidence(
        self, trading_engine, sample_market_data_with_bs
    ):
    ...
    def test_filter_sell_opportunities_low_confidence_excluded(
        self, trading_engine, sample_market_data_with_bs
    ):
    ...
    def test_filter_sell_opportunities_nan_confidence_handled(
        self, trading_engine, sample_market_data_with_bs
    ):
...
class TestPortfolioFiltering:
    ...
    def test_apply_portfolio_filters_ticker_column_variations(
        self, trading_engine, sample_market_data_with_bs
    ):
        """Test portfolio filtering works with different ticker column names."""
        opportunities = {
            "buy_opportunities": trading_engine.filter_service.filter_buy_opportunities(
                sample_market_data_with_bs
            ),
            "sell_opportunities": trading_engine.filter_service.filter_sell_opportunities(
                sample_market_data_with_bs
            ),
            "hold_opportunities": trading_engine.filter_service.filter_hold_opportunities(
                sample_market_data_with_bs
            ),
        }
        ...
        for col_name in column_variations:
            portfolio_df = pd.DataFrame({col_name: ["AAPL", "MSFT"], "quantity": [100, 50]})
    ...
    def test_apply_portfolio_filters_with_portfolio_bs_signals(
        self, trading_engine, sample_market_data_with_bs, sample_portfolio_with_bs
    ):
        ...
        sell_log_calls = [
            call for call in mock_logger.call_args_list if "portfolio SELL stocks" in str(call)
        ]
        assert len(sell_log_calls) >= 1

        # Should log addition of portfolio HOLD stocks
        hold_log_calls = [
            call for call in mock_logger.call_args_list if "portfolio HOLD stocks" in str(call)
        ]
        assert len(hold_log_calls) >= 1

    def test_apply_portfolio_filters_ticker_equivalence(
        self, trading_engine, sample_market_data_with_bs
    ):
    ...
    def test_apply_portfolio_filters_error_handling(
        self, trading_engine, sample_market_data_with_bs
    ):
        ...
        result = trading_engine.portfolio_service.apply_portfolio_filters(
            opportunities, invalid_portfolio
        )
...
class TestConfidenceScoreCalculation:
    ...
    def test_calculate_confidence_score_analyst_boost(self, trading_engine):
        ...
        assert scores.iloc[1] == pytest.approx(
            0.9
        )  # 5 analysts: 0.6 + 0.2 + 0.1 (total_ratings >= 5)
        assert scores.iloc[2] == pytest.approx(
            1.0
        )  # 10 analysts: 0.6 + 0.2 + 0.1 + 0.1 = 1.0 capped
        assert scores.iloc[3] == pytest.approx(
            1.0
        )  # 15 analysts: 0.6 + 0.2 + 0.1 + 0.1 = 1.0 capped
    ...
    def test_calculate_confidence_score_missing_data_penalty(self, trading_engine):
        ...
        assert scores.iloc[2] == pytest.approx(
            expected_base - 0.2
        )  # Missing buy_percentage penalty
    ...
    def test_calculate_confidence_score_error_handling(self, trading_engine):
        ...
        df = pd.DataFrame({"analyst_count": ["invalid", None, 5], "bad_column": [1, 2, 3]})
...
class TestNotradeFiltering:
    ...
    def test_filter_notrade_tickers_basic(
        self, trading_engine, sample_market_data_with_bs, temp_notrade_file
    ):
        ...
        with patch.object(trading_engine.logger, "info") as mock_logger:
            result = trading_engine.filter_service.filter_notrade_tickers(
                sample_market_data_with_bs, temp_notrade_file
            )
        ...
        filter_calls = [call for call in mock_logger.call_args_list if "Filtered out" in str(call)]
        assert len(filter_calls) == 1

    def test_filter_notrade_tickers_equivalence(
        self, trading_engine, sample_market_data_with_bs, temp_notrade_file
    ):
    ...
    def test_filter_notrade_tickers_missing_file(self, trading_engine, sample_market_data_with_bs):
        ...
        result = trading_engine.filter_service.filter_notrade_tickers(
            sample_market_data_with_bs, "nonexistent.csv"
        )
...
class TestTradingSignalCalculation:
    ...
    def test_calculate_trading_signals_calls_functions(
        self, mock_action, mock_exret, mock_expected, trading_engine
    ):
        ...
        with patch.object(
            trading_engine.analysis_service,
    ...
    def test_calculate_trading_signals_adds_confidence(self, trading_engine):
        ...
        with (
            patch("trade_modules.analysis_service.calculate_expected_return", return_value=df),
            ...
            patch.object(
                trading_engine.analysis_service,
                "calculate_confidence_score",
                return_value=pd.Series([0.8], index=df.index),
            ),
        ):
    ...
    def test_calculate_trading_signals_error_handling(self, trading_engine):
        ...
        with patch(
            "trade_modules.analysis_service.calculate_expected_return",
...
class TestAsyncBatchProcessing:
    ...
    async def test_process_ticker_batch_basic(self, trading_engine):
        ...
        batch_1_results = [{"ticker": "AAPL", "price": 150.0}, {"ticker": "MSFT", "price": 280.0}]
        batch_2_results = [{"ticker": "GOOGL", "price": 2750.0}]

        with patch.object(
            trading_engine.data_processing_service,
            "_process_batch",
            side_effect=[batch_1_results, batch_2_results],
        ):
    ...
    async def test_process_batch_error_handling(self, trading_engine):
        ...
        with patch.object(
            trading_engine.data_processing_service, "_process_single_ticker"
        ) as mock_single:
    ...
    async def test_process_single_ticker_success(self, trading_engine):
        ...
        with (
            patch(
                "trade_modules.data_processing_service.process_ticker_input", return_value="AAPL"
            ),
            patch(
                "trade_modules.data_processing_service.get_ticker_for_display", return_value="AAPL"
            ),
        ):
    ...
    async def test_process_single_ticker_no_data(self, trading_engine):
        ...
        with patch(
            "trade_modules.data_processing_service.process_ticker_input", return_value="INVALID"
        ):
            result = await trading_engine.data_processing_service._process_single_ticker(ticker)
        ...
        market_data = pd.DataFrame(
            {
                ...
                "upside": [
                    ...
                    20.0,
                    -8.0,
                ],  # NVDA/AMZN/MSFT BUY, AAPL HOLD (below min_upside=10), GOOGL SELL
...
class TestErrorHandlingAndEdgeCases:
    ...
    async def test_batch_processing_all_failures(self, trading_engine):
        ...
        with patch.object(
            trading_engine.data_processing_service, "_process_single_ticker", return_value=None
        ):
            result = await trading_engine.process_ticker_batch(tickers)
