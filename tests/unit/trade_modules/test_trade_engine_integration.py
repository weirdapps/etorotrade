...
class TestRealDataIntegration:
    ...
    @pytest.mark.asyncio
    async def test_full_workflow_real_csv_structure(
        self, market_data_with_prices, real_portfolio_csv_data
    ):
        ...
        result = await engine.analyze_market_opportunities(
            market_data_with_prices, portfolio_df=real_portfolio_csv_data
        )
    ...
    async def test_notrade_filtering_real_data(self, market_data_with_prices, temp_csv_files):
        ...
        result = await engine.analyze_market_opportunities(
            market_with_notrade, notrade_path=temp_csv_files["notrade"]
        )
    ...
    async def test_portfolio_column_variations(self, market_data_with_prices):
        ...
        for col_name, tickers in column_variations:
            portfolio_df = pd.DataFrame(
                {col_name: tickers, "quantity": [100, 50, 75], "value": [80000, 9265, 30768]}
            )

            result = await engine.analyze_market_opportunities(
                market_data_with_prices, portfolio_df=portfolio_df
            )
    ...
    async def test_international_ticker_handling(self, market_data_with_prices):
        ...
        result = await engine.analyze_market_opportunities(
            combined_data, portfolio_df=intl_portfolio
        )
...
class TestDataValidationIntegration:
    ...
    async def test_missing_required_columns_handling(self):
        ...
        incomplete_data = pd.DataFrame(
            {"symbol": ["AAPL", "MSFT"], "name": ["Apple", "Microsoft"], "BS": ["B", "H"]}
        ).set_index("symbol")
...
class TestPerformanceIntegration:
    ...
    async def test_large_dataset_performance(self):
        ...
        # Should complete in reasonable time for 1000 stocks.
        ...
        assert processing_time < 120.0, f"Processing took {processing_time:.2f}s, expected < 120.0s"
    ...
    async def test_batch_processing_performance(self):
        ...
        with (
            patch(
                "trade_modules.data_processing_service.process_ticker_input",
                side_effect=lambda x: x,
            ),
            patch(
                "trade_modules.data_processing_service.get_ticker_for_display",
                side_effect=lambda x: x,
            ),
        ):
            import time
