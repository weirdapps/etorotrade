...
def notrade_csv_data(input_data_path):
    ...
    if notrade_path.exists():
        ...
        # Fallback data if file doesn't exist
        return pd.DataFrame({"symbol": ["BANNED1", "BANNED2"]})
...
def enhanced_market_data(market_csv_data):
    ...
    enhanced["BS"] = np.where(
        enhanced["upside"] > 15, "B", np.where(enhanced["upside"] < 5, "S", "H")
    )
...
class TestBehavioralValidation:
    ...
    @pytest.mark.asyncio
    async def test_portfolio_filtering_baseline_behavior(
        self, enhanced_market_data, portfolio_csv_data
    ):
        ...
        result_with_portfolio = await engine.analyze_market_opportunities(
            enhanced_market_data, portfolio_df=portfolio_csv_data
        )

        # Portfolio filtering should reduce buy opportunities
        # (portfolio holdings should be excluded from buy)
        assert len(result_with_portfolio["buy_opportunities"]) <= len(
            result_no_portfolio["buy_opportunities"]
        )
    ...
    async def test_notrade_filtering_baseline_behavior(
        self, enhanced_market_data, notrade_csv_data, input_data_path
    ):
        ...
        try:
            ...
            result = await engine.analyze_market_opportunities(
                enhanced_market_data, notrade_path=str(notrade_path)
            )
    ...
    @pytest.mark.asyncio
    @pytest.mark.skip(
        reason="Log message 'Using ACT column values as BS column' not implemented in trade_engine"
    )
    ...
    def test_position_sizer_baseline_behavior(self):
        ...
        market_data = {"price": 150.0, "beta": 1.2, "market_cap": 2.5e12}
        ...
        high_beta_position = sizer.calculate_position_size(
            "HIGH_BETA", high_beta_data, portfolio_value
        )
        low_beta_position = sizer.calculate_position_size(
            "LOW_BETA", low_beta_data, portfolio_value
        )
...
class TestBehavioralRegression:
    ...
    @pytest.mark.asyncio
    @pytest.mark.skip(
        reason="Enhanced SELL scoring system now uses multi-factor scoring instead of simple confidence thresholds"
    )
    ...
    @pytest.mark.skip(
        reason="Implementation detail test - equivalence checking happens in portfolio_service"
    )
    @pytest.mark.asyncio
    async def test_portfolio_ticker_equivalence_checking(self, enhanced_market_data):
        ...
        with patch("yahoofinance.utils.data.ticker_utils.are_equivalent_tickers") as mock_equiv:
            ...
            mock_equiv.side_effect = lambda t1, t2: (
                (t1 == "AAPL" and t2 == "AAPL.US") or (t1 == "AAPL.US" and t2 == "AAPL") or t1 == t2
            )

            result = await engine.analyze_market_opportunities(
                enhanced_market_data, portfolio_df=portfolio_data
            )
    ...
    async def test_logging_behavior_consistency(self, enhanced_market_data):
        ...
        summary_calls = [
            call for call in mock_info.call_args_list if "Analysis complete" in str(call)
        ]
