...
class TestAnalysisEngineComprehensive(unittest.TestCase):
    ...
    def test_data_type_conversions(self):
        ...
        perc_df["short_percent"] = perc_df["short_percent"].apply(
    ...
    def test_negative_upside_never_buy_safety_check(self):
        ...
        self.assertNotEqual(
            actions["NEG_HIGH_CONSENSUS"],
            "B",
            "SAFETY FAILURE: -20% upside marked as BUY despite high consensus",
        )
        self.assertNotEqual(
            actions["NEG_LOW_CONSENSUS"], "B", "SAFETY FAILURE: -5% upside marked as BUY"
        )

        # Zero upside should also not be BUY (no profit potential)
        self.assertNotEqual(
            actions["ZERO_UPSIDE"], "B", "Zero upside marked as BUY - no profit potential"
        )
