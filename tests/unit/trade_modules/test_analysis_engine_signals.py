...
class TestExretCalculation:
    ...
    def test_exret_positive_upside_high_buy_percentage(self):
        ...
        df = pd.DataFrame([{"ticker": "TEST", "upside": 10.0, "buy_percentage": 70.0}]).set_index(
            "ticker"
        )
    ...
    def test_exret_zero_buy_percentage(self):
        ...
        df = pd.DataFrame([{"ticker": "TEST", "upside": 15.0, "buy_percentage": 0.0}]).set_index(
            "ticker"
        )
    ...
    def test_exret_negative_upside(self):
        ...
        df = pd.DataFrame([{"ticker": "TEST", "upside": -5.0, "buy_percentage": 60.0}]).set_index(
            "ticker"
        )
