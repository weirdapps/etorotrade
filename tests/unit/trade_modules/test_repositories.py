...
from trade_modules.repositories import CsvRepository, MarketDataRepository, PortfolioRepository
...
class TestCsvRepository:
    ...
    def sample_data(self):
        ...
        return pd.DataFrame(
            {"price": [100.0, 200.0, 300.0], "volume": [1000, 2000, 3000]},
    ...
    def test_csv_parameters_compatibility(self, csv_repo, temp_dir):
        ...
        sample_df = pd.DataFrame(
            {"price": [100, 200], "volume": [1000, 2000]}, index=["AAPL", "MSFT"]
        )
...
class TestRepositoryIntegration:
    ...
    def test_backward_compatibility_with_existing_patterns(self, temp_dir):
        ...
        portfolio_data = pd.DataFrame(
            {"Quantity": [100, 200], "Value": [10000, 40000]}, index=["AAPL", "MSFT"]
        )

        market_data = pd.DataFrame(
            {"price": [100, 200], "upside": [10, 15]}, index=["AAPL", "MSFT"]
        )
