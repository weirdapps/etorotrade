...
class MockYahooFinanceResponses:
    ...
    def get_market_data_response(tickers: list[str]) -> dict[str, dict[str, Any]]:
        ...
        return {
            ticker: MockYahooFinanceResponses.get_ticker_info_response(ticker) for ticker in tickers
        }
...
class MockProviders:
    ...
    def create_mock_async_provider():
        ...
        mock_provider.get_ticker_info.return_value = (
            MockYahooFinanceResponses.get_ticker_info_response()
        )
        mock_provider.get_market_data.return_value = (
            MockYahooFinanceResponses.get_market_data_response(["AAPL", "MSFT"])
        )
    ...
    def create_mock_sync_provider():
        ...
        mock_provider.get_tickers_info.return_value = (
            MockYahooFinanceResponses.get_market_data_response(["AAPL", "MSFT"])
        )
        mock_provider.is_available.return_value = True
        return mock_provider
...
def patch_yahoo_finance_api():
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            with patch(
                "yahoofinance.api.providers.async_hybrid_provider.AsyncHybridProvider"
            ) as mock_provider:
