...
class TestTradingEngineIntegration:
    ...
    def test_calculate_exret_integration(self, sample_market_data):
        ...
        assert result.loc[result["ticker"] == "AAPL", "EXRET"].iloc[0] == pytest.approx(
            expected_aapl, abs=0.1
        )
        assert result.loc[result["ticker"] == "MSFT", "EXRET"].iloc[0] == pytest.approx(
            expected_msft, abs=0.1
        )
        assert result.loc[result["ticker"] == "GOOGL", "EXRET"].iloc[0] == pytest.approx(
            expected_googl, abs=0.1
        )
    ...
    @pytest.mark.asyncio
    async def test_analyze_market_opportunities_integration(
        self, sample_market_data, mock_provider
    ):
